...
// little. Hint: Some dogs have multiple owners, so you first need to find Sarah in
// the owners array, and so this one is a bit tricky (on purpose) �
const sarahDog = dogs.find(dog => dog.owners.includes('Sarah')); // Retorna objeto
...
if (
  ...
  console.log(
    'Isso mesmo !! Você vem alimentando seu cachorro com a porção recomendada'
  );
else if (sarahDog.curFood > sarahDog.recommendedFood)
  console.log(
    `Você quer matar seu cachorro com tanta comida? Reduza a porção para a recomendada (${sarahDog.recommendedFood}g)`
  );
else if (sarahDog.curFood < sarahDog.recommendedFood)
  console.log(
    `Tadin do seu cachorro, ele esta passando fome !! Aumente a porção para a recomendada (${sarahDog.recommendedFood}g) `
  );
...
// that is recommended (just true or false)
console.log(
  `\n\nThere is any dog eating exactly the amount of food that is recommended: ${dogs.some(
    dog => dog.curFood === dog.recommendedFood
  )}`
);
...
console.log(
  `There is any dog eating an okay amount of food: ${dogs.some(
    dog =>
...
// Creatig shallow copy using .splice("") method
